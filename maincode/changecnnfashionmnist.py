...
def load_data(batch_size):
    ...
    train_dataset = datasets.CIFAR10(root='./data', train=True, transform=data_transform, download=True)  # 指定训练集
    test_dataset = datasets.CIFAR10(root='./data', train=False, transform=data_transform, download=True)  # 指定测试集
...
class SimpleCNN(nn.Module):
    def __init__(self, weight_path, gradient_path, history_path, __PATH__):
        ...
        self.fc = nn.Linear(32*32*32, 10)  # Assuming input image size is 28x28
...
def _initialize_weights(self):
    for m in self.modules():
        if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d):
            # Initialize the whole tensor with Xavier initialization
            nn.init.xavier_normal_(m.weight.data)
            
            # Set 90% of weights to 0
