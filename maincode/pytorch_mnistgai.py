    ...
    def train(self, epoch_num, lr, criterion, opt):
        ...
        for i in range(epoch_num):
            for data, label in train_loader:
                ...
                train_loss=loss
                # 获取当前fc1.weight的平均值
                current_fc1_weight_avg = torch.mean(self.fc1.weight.data)

            # 如果是第一个 epoch，保存当前的平均值
                if i == 0 and previous_fc1_weight_avg is None:
                    previous_fc1_weight_avg = current_fc1_weight_avg

            # 如果是第二个 epoch，比较当前平均值和第一个 epoch 的平均值的差值
                elif i == 1:
                    diff = abs(current_fc1_weight_avg - previous_fc1_weight_avg)
                    if diff < 0.014:
                        print(f"Stopping training early after 2 epochs due to small weight difference: {diff}")
                        return None  # 停止训练并返回 None
            ...
            weight_dic["model_epoch"+str(i+1)] = copy.deepcopy(net.state_dict())
...
def predict_test(i, j): 
    ...
    path = f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/cnnmodel.pkl'
...
if __name__ == '__main__':
    ...
    node1=5
    node2=5
    total_stopped_count = 0 # 记录总共提前停止的次数
    total_stopped_time = 0 # 记录总共提前停止的时间
    #-------------------------------------------------------------------------
    for i in range(1):
        j = 0
        while j < 10:
            start_time_iteration = time.time() # 开始计时当前迭代
            train_loader, test_loader, valid_loader = load_data(batch_size)
            criterion = nn.CrossEntropyLoss()#nn.CrossEntropyLoss()
            
            __PATH__=f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/wholetrainhistory.pkl'
            weight_path = f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/weight_dic.pkl'
            gradient_path = f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/gradient_dic.pkl'
            history_path = f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/train_history_dic.pkl'
            net =Mlp(weight_path, gradient_path, history_path,__PATH__)
            net.apply(_initialize_weights)
            f1= f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/weights.txt'
            f2= f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/gradients.txt'
            f3= f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/trainhistory.txt'
            ...
            if train_time is None: # 如果训练提前停止，重新开始同一次迭代的训练
                total_stopped_count += 1
                stopped_time_iteration = time.time() - start_time_iteration # 计算当前迭代提前停止的时间
                total_stopped_time += stopped_time_iteration
                print(f"Training stopped early in iteration {j}. Restarting iteration {j}.")
                continue
            ...
            torch.save(net_state_dict,f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/train_result{j+1}/cnnmodel.pkl')  # 保存整个模型
        ...
        **************************************************************  
            ...
            a=[test_accuracy]        
            print(a)
            csvfilesave(f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/accuracy.csv',a)
            csvfilesave(f'/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultnew/b100l5lr0001-{i+1}/accuracy.csv',a)
            j += 1 
            #---------------------------------varify code----------------------------------------------------
            predict_test(i,j)
    print(f"Total number of early stopped iterations: {total_stopped_count}")
    print(f"Total time lost due to early stopping: {total_stopped_time} seconds")    
