...
def painthistcumulative(fc_weight_max, fc_weight_mid, fc_weight_min, figure_name):
    ...
    n_bins = 100
    range_min = min(min(fc_weight_max), min(fc_weight_mid), min(fc_weight_min))
    range_max = max(max(fc_weight_max), max(fc_weight_mid), max(fc_weight_min))
    bins = np.linspace(range_min, range_max, n_bins)
    
    # Compute cumulative frequency distributions
    cumfreq_max, _ = np.histogram(fc_weight_max, bins=bins, density=True)
    cumfreq_mid, _ = np.histogram(fc_weight_mid, bins=bins, density=True)
    cumfreq_min, _ = np.histogram(fc_weight_min, bins=bins, density=True)
    cumfreq_max = np.cumsum(cumfreq_max)
    cumfreq_mid = np.cumsum(cumfreq_mid)
    cumfreq_min = np.cumsum(cumfreq_min)

    # Plot overlaid histograms
    fig, ax = plt.subplots()
    ax.plot(bins[:-1], cumfreq_max, label='Max accuracy', color='gold')
    ax.plot(bins[:-1], cumfreq_mid, label='Mid accuracy', color='lightgreen')
    ax.plot(bins[:-1], cumfreq_min, label='Min accuracy', color='lightskyblue')

    # Set plot parameters
    ax.set_title('Gradient Cumulative Histogram', fontsize=12)
    ax.set_xlabel('Gradients', fontsize=10)
    ...
    # Save and show plot
    plt.tight_layout()
    plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/result/Gradient_cumulative_overlap2.png", dpi=300)
    plt.show()
...
painthistcumulative(all_max_weight,all_mid_weight,all_min_weight,"gradients cumulative frequency")
