...
indices1 = sorted(range(len(test_acc_diff1)), key=lambda i: test_acc_diff1[i])[:1]
# use the indices to get the corresponding paths
model_paths3 = [model_paths3[i] for i in indices1]
indices2 = sorted(range(len(test_acc_diff2)), key=lambda i: test_acc_diff2[i])[:1]
# use the indices to get the corresponding paths
...
painthistoverlap(all_max_weight,all_min_weight,4)
plt.tight_layout()
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultb100l5lr0001-cifar10plot/positive_gradients_distribution_overlap.png")
plt.show()