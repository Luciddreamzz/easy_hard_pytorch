...
def nodesproperty(weight, layer1, layer2):
    z_sum = 0
    disparities = []

    for out_channel in range(weight.shape[0]):
        s = weight[out_channel].sum()
        for in_channel in range(weight.shape[1]):
            for i in range(weight.shape[2]):
                for j in range(weight.shape[3]):
                    z = (abs(weight[out_channel][in_channel][i][j]) / abs(s)) ** 2
                    z_sum += z
        disparities.append(z_sum)
        z_sum = 0

    return disparities

def node_strength(weight,layer1, layer2):
    strengths = []
    ...
    for out_channel in range(weight.shape[0]):
        ...
        for in_channel in range(weight.shape[1]):
            for i in range(weight.shape[2]):
                for j in range(weight.shape[3]):
                    strength += abs(weight[out_channel][in_channel][i][j])
    ...
    for idx, model_paths in enumerate(all_model_paths):
        ...
        for i, path in enumerate(model_paths):  
            ...
            for k in range(1, 21):
                ...
                if ax_index==0:
                    z_values= node_strength(tensor1,input_size,node1)
                if ax_index==1:
                    z_values= node_strength(tensor1,node1,node2)
                if ax_index==2:
                    z_values= node_strength(tensor1,node2,output_size)
                ...
                experiment_weight_mean_values.extend(z_values)
...
# 保存图形
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/result-cnn64-7-plot/nodestrength_lineplot_allori.png", dpi=300)
