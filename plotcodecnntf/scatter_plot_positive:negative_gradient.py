...
def addweight(axs, path1, path2, vmin, vmax,flag):
    ...
    dod2 = torch.load(f)
    torch.set_printoptions(threshold=np.inf) #Display all data for complex matrix
    tensor1=getgradient(dod1,"model_epoch20 conv1.weight").flatten()
    ...
    tensor2 = getgradient(dod1,"model_epoch20 conv2.weight").flatten()
...
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/result-cnn64-7-plot/mean_std_scatter_negative_gradient.png", dpi=300)
plt.show()
