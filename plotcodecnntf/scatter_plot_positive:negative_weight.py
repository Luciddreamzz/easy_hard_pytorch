...
def addweight(axs, path1, path2, vmin, vmax,flag):
    ...
    tensor1 = (dod1["conv1.weight"]).flatten()
    ...
    tensor2 = (dod1["conv2.weight"]).flatten()
    ...
    all_positive_mean = torch.cat((fc1_positive, fc2_positive), dim=0).flatten().mean()
    all_negative_mean = torch.cat((fc1_negative, fc2_negative), dim=0).flatten().mean()
    all_positive_std = torch.cat((fc1_positive, fc2_positive), dim=0).flatten().std()
    all_negative_std = torch.cat((fc1_negative, fc2_negative), dim=0).flatten().std()
    ...
    #     scatter_with_accuracy(all_positive_mean, all_positive_std, accuracy, axs[2], vmin, vmax,accuracy_type='min accuracy') 
    # if flag==2:
    ...
    #     scatter_with_accuracy(all_positive_mean, all_positive_std, accuracy, axs[2], vmin, vmax,accuracy_type='mid accuracy') 
    # else:
    ...
    #     scatter_with_accuracy(all_positive_mean, all_positive_std, accuracy, axs[2], vmin, vmax,accuracy_type='max accuracy')
    ...
    if flag==1:
        ...
        scatter_with_accuracy(all_negative_mean, all_negative_std, accuracy, axs[2], vmin, vmax,accuracy_type='min accuracy') 
    if flag==2:
        ...
        scatter_with_accuracy(all_negative_mean, all_negative_std, accuracy, axs[2], vmin, vmax,accuracy_type='mid accuracy') 
    else:
        ...
        scatter_with_accuracy(all_negative_mean, all_negative_std, accuracy, axs[2], vmin, vmax,accuracy_type='max accuracy')
...
for i in range(13,22):
    for j in range(10):
        path1 = f"/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultcnn/b100l5lr0001-{i+1}/train_result{j+1}/cnnmodel.pkl"
...
for i in range(13, 22):
    for j in range(1,11):
        path = "/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultcnn/b100l5lr0001-{}/train_result{}/cnnmodel.pkl".format(i, j)
...
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/result-cnn64-7-plot/mean_std_scatter_negative.png", dpi=300)
plt.show()
