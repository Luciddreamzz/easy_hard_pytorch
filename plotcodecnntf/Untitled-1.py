...
for model_paths in all_model_paths:
    ...
    for i, path in enumerate(model_paths):  
        ...
        for k in range(1, 21):
            ...
            z_values, _ = nodesproperty(tensor1, node1, node2, 1)
            if isinstance(z_values, float):
                z_values = [z_values]
            # Convert to numpy array, filter out inf values and apply log transformation
            z_values = np.array(z_values)
            z_values = z_values[np.isfinite(z_values)]
            z_values = np.log1p(z_values) # log transformation
...
ax.set_ylabel('Node Disparity (log-transformed)', fontsize=10) # Change ylabel here
ax.tick_params(axis='both', labelsize=8)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=0, vmax=1))
cbar = plt.colorbar(sm)

plt.tight_layout()
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultb100l5lr0001-mnist-plot/nodedisparity_lineplot_alllogf1.png", dpi=300)
plt.show()
