...
def addweight(axs, path1, path2, vmin, vmax,flag):
    ...
    all_mean = torch.cat((tensor1, tensor2), dim=0).mean()
    all_std = torch.cat((tensor1, tensor2), dim=0).std()
...
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/result-cnn64-7-plot/mean_std_scatter.png", dpi=300)
plt.show()
