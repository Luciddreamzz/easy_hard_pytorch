...
for ax_index, layer in enumerate(layers):
    ...
    ax.set_ylabel(f'{layer} node strength', fontsize=10)
    ...
    for idx, model_paths in enumerate(all_model_paths):
        ...
        for i, path in enumerate(model_paths):  
            ...
            for k in range(1, 21):
                ...
                if ax_index==0:
                    _,s_values= nodesproperty(tensor1,input_size,node1,flag=1)
                if ax_index==1:
                    _,s_values= nodesproperty(tensor1,node1,node2,flag=1)
                if ax_index==2:
                    _,s_values= nodesproperty(tensor1,node2,output_size,flag=1)
            ...
            print(experiment_weight_mean_values)
        ...
        # if len(epoch_values) != len(mean_values):
        #     print(f"Error in {path}: epoch_values and mean_values length mismatch!")
        #     continue
...
# # 保存图形
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultb100l5lr0001-fashionmnist-plot/nodestrength3clus_lineplot_all.png", dpi=300)
