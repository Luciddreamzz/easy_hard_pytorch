...
def node_resistance(dictionary, axis):
    return dictionary.abs().sum(axis=axis)

def resistance_curvature(dictionary, layer1, layer2, flag):
    kappa_values = []
    # print(dictionary.shape,layer1,layer2)
    p_out=dictionary.abs().sum(axis=1)
    p_in=dictionary.abs().sum(axis=0)
    for i in range(layer2):
        for j in range(layer1):
            if abs(dictionary[i][j]) != 0:  # 避免除以零
                kappa = 2 * (p_out[i] + p_in[j]) / abs(dictionary[i][j])
                kappa_values.append(kappa)
    return torch.sum(p_out).item(),np.mean(kappa_values)
...
for i in range(1, 101):
    for j in range(1, 11):
        path = "/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultb100l5lr0001-fashionmnist/b100l5lr0001-{}/train_result{}/weights.txt".format(i, j)
...
for ax_index, layer in enumerate(layers):
    ...
    ax.set_ylabel(f'{layer} node disparity', fontsize=10)
    ax.tick_params(axis='both', labelsize=8)

    # Group all model paths
    all_model_paths = [model_paths1,model_paths2,model_paths4]
    ...
    for idx, model_paths in enumerate(all_model_paths):
        ...
        for i, path in enumerate(model_paths):  
            ...
            for k in range(1, 21):
                ...
                if ax_index==0:
                    _,s_values= resistance_curvature(tensor1,input_size,node1,flag=1)
                if ax_index==1:
                    _,s_values= resistance_curvature(tensor1,node1,node2,flag=1)
                if ax_index==2:
                    _,s_values= resistance_curvature(tensor1,node2,output_size,flag=1)
                if isinstance(s_values, float):
                    s_values = [s_values]
                # Convert to numpy array and filter out inf values
                s_values = np.array(s_values)
                s_values = s_values[np.isfinite(s_values)]
                s_values = np.clip(s_values, None, 1000000)  # Clip at upper bound 1000000
                # Apply log transformation
                s_values = np.log1p(s_values)
                experiment_weight_mean_values.extend(s_values.tolist())
...
# 保存图形
plt.savefig("/Users/jianqiaolong/Downloads/hr-mnist-pytorch/resultb100l5lr0001-fashionmnist-plot/resistance_curvature3clus_lineplot_allcliplog.png", dpi=300)
